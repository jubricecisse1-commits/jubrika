-- ============================================================
-- JUBRIKA — SETUP COMPLET (schema + produits)
-- Copie tout ce fichier et colle-le dans Supabase SQL Editor
-- ============================================================
...
SELECT s.id, s.commercial_id, p.nom_complet AS commercial_nom, p.ville AS commercial_ville,
  s.produit_id, pr.nom AS produit_nom, pr.reference AS produit_reference,
  pr.categorie AS produit_categorie, pr.prix_base, pr.photo_url,
...
SELECT v.id, v.commercial_id, p.nom_complet AS commercial_nom, p.ville AS commercial_ville,
  v.client_telephone, v.client_ville, v.date_vente, v.montant_total,
  v.montant_apres_remise, v.reduction_appliquee, cp.code AS code_promo,
  v.mode_paiement, v.notes, v.statut, v.created_at
FROM ventes v
...
CREATE TRIGGER codes_promo_updated_at BEFORE UPDATE ON codes_promo FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE TRIGGER transferts_updated_at BEFORE UPDATE ON transferts_commandes FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_decrement_stock AFTER INSERT ON vente_produits FOR EACH ROW EXECUTE FUNCTION decrementer_stock_vente();
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_increment_stock AFTER INSERT ON distributions FOR EACH ROW EXECUTE FUNCTION incrementer_stock_distribution();

CREATE OR REPLACE FUNCTION incrementer_utilisation_promo()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.code_promo_id IS NOT NULL THEN
    UPDATE codes_promo SET nb_utilisations_actuel = nb_utilisations_actuel + 1 WHERE id = NEW.code_promo_id;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_increment_promo AFTER INSERT ON ventes FOR EACH ROW EXECUTE FUNCTION incrementer_utilisation_promo();
...
CREATE POLICY "profiles_select_own" ON profiles FOR SELECT USING (auth.uid() = id OR (SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');
...
CREATE POLICY "ventes_select" ON ventes FOR SELECT USING (commercial_id = auth.uid() OR (SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');
CREATE POLICY "ventes_insert" ON ventes FOR INSERT WITH CHECK (commercial_id = auth.uid());
CREATE POLICY "stocks_select" ON stocks FOR SELECT USING (commercial_id = auth.uid() OR (SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');
CREATE POLICY "produits_select" ON produits FOR SELECT USING (actif = TRUE);
CREATE POLICY "produits_admin" ON produits FOR ALL USING ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');
CREATE POLICY "notifications_own" ON notifications FOR ALL USING (user_id = auth.uid());
CREATE POLICY "promos_select" ON codes_promo FOR SELECT USING (actif = TRUE OR (SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');
CREATE POLICY "promos_admin" ON codes_promo FOR ALL USING ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');
CREATE POLICY "vente_produits_select" ON vente_produits FOR SELECT USING ((SELECT commercial_id FROM ventes WHERE id = vente_id) = auth.uid() OR (SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');
CREATE POLICY "vente_produits_insert" ON vente_produits FOR INSERT WITH CHECK ((SELECT commercial_id FROM ventes WHERE id = vente_id) = auth.uid());
CREATE POLICY "transferts_select" ON transferts_commandes FOR SELECT USING (commercial_expediteur_id = auth.uid() OR commercial_destinataire_id = auth.uid() OR (SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');
...
-- PRODUITS JUBRIKA (identifiés dans la base de ventes Cameroun)
INSERT INTO produits (nom, reference, categorie, prix_base, description, actif) VALUES
  ('Lunette Noir T1',        'LUN-T1',   'lunettes_style',            7500,  'Lunette noire unisexe T1', true),
  ('Lunette Noir T2',        'LUN-T2',   'lunettes_style',           10000,  'Lunette noire unisexe T2', true),
  ('Lunette Noire T3',       'LUN-T3',   'lunettes_style',           11000,  'Lunette noire unisexe T3', true),
  ('Lunette Noir T4',        'LUN-T4',   'lunettes_style',           10000,  'Lunette noire unisexe T4', true),
  ('Lunette Bleu D1',        'LUN-D1',   'lunettes_style',           15000,  'Lunette bleue D1', true),
  ('Lunettes Chrome H1',     'PHOT-H1',  'lunettes_photochromiques', 15000,  'Lunettes photochromiques Chrome H1', true),
  ('Lunettes Chrome H2',     'PHOT-H2',  'lunettes_photochromiques', 11000,  'Lunettes photochromiques Chrome H2', true),
  ('Lunettes Réglable P1',   'LUN-P1',   'lunettes_style',           16500,  'Lunettes réglables P1', true),
  ('Lunettes Pliables S1',   'LUN-S1',   'lunettes_style',            8000,  'Lunettes pliables S1', true),
  ('Lunette Noire F1 Femme', 'LUN-F1',   'lunettes_style',            8500,  'Lunette noire femme F1', true),
  ('Lunette Noire F2 Femme', 'LUN-F2',   'lunettes_style',            8500,  'Lunette noire femme F2', true),
  ('Lunettes Conduite',      'LUN-COND', 'lunettes_style',            7000,  'Lunettes de conduite', true),
  ('Montre SABR',            'MON-SABR', 'montres',                  15000,  'Montre SABR luxe', true),
  ('Montre Instang',         'MON-INST', 'montres',                  16500,  'Montre Instang', true)
